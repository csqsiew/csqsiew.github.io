...
library(igraph)
library(igraphdata) # karate is a network object available from this package
library(tidyverse)

data('karate') # notice that a karate object is in your environment 
...
In the `karate_node_added.csv` file, sample node attributes for the `karate` network have been added (it is as if I conducted a demographic survey with these individuals). The code below shows you how to read in this data and add this information as node attributes to the network using the `set_vertex_attribute` function. 
...
# this code "arranges" the dataframe so that the information is ordered based on the node order in the network
...
When you run `summary(karate)` you should see that there are additional labels (gender, belt, age) included in the attributes section. 
...
Note that `make_ego_graph` returns a special "list" object. The `[[1]]` appended to it simply means that we want to extract the first element of the list, which is a network object. In the examples above, the list only contains a single network object. However, it is possible to extract the ego graphs of multiple individuals (e.g., `make_ego_graph(karate, order = 1, nodes = c('Actor 3', 'Actor 2'))`) which will give you a list of two network objects, the ego graph of Actor 3 and Actor 2. Then you can use [[1]] and [[2]] to extract the networks specifically. 
...
1. How would you include this extra information about the frequency of interaction directly into the original raw data file? (Hint: The edge list approach is better for this, and you can put in some made up numbers from 1 to 10 to depict frequency of interaction). 

2. Create a network object that includes this new information. (Hint: Is interaction frequency a node or edge attribute?)

3. Janelle has a .csv file containing the ages of the cats. How can you create a network object that includes this information? (Hint: Is cat age a node or edge attribute? Do you need to create a second file containing this information (feel free to make up some numeric data!)?) 

4. Subset the network to only show the 4 oldest cats. How many edges does this network have?

5. Subset the network to only show cats that interacted frequently with each other. How many nodes and edges does this network have? 

6. Create a network visualization of the entire cat network and make at least one modification to the default plot (see Chapter 14 for ideas). For instance, can you color the nodes based on the cat's ages? 
