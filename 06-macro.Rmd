...
Because `karate_uu` is undirected and unweighted, the result is unsurprisingly identical across all 4 versions (try this with a directed, weighted network!). When a graph contains edge weights, then they are automatically considered unless you indicate otherwise using `weights = NA`. Use the `directed` argument to specify if the direction of the edges should be considered in the computation of short paths. If this is selected, then only short paths whose directions are aligned are considered in the computation (A->B->C is a valid path connecting nodes A and C but not A->B<-C). 

An important point to note is that edge weights are interpreted as *distances* rather than *strength of connection*, as weighted short paths are computed by summing of the edge weights. Hence, a longer path is interpreted as being more distant, or further apart. See Chapter 7 for more information about this. 
...
Another variant is to compute the *average local clustering coefficient*, analogous to how mean degree is computed above. Below, we compute the local C (which is a measure of the level of interconnectedness among a target node's neighbors) for each node, then take the mean. 

```{r}
#| echo: true
#| eval: true

transitivity(karate_uu, type = 'local') |> mean()
```
...
**Assortative mixing** is a measure of *homophily* in the network, based on some internal/external property of the nodes. Do nodes that are connected to each other tend to have similar properties?

### Assortative Mixing by Degree (continuous attributes)

A common extension of assortative mixing is **assortative mixing by degree**, which measures the correlation of degrees between neighboring nodes. Do nodes that are connected to each other tend to have similar degrees?
...
There is *dissortative mixing by degree* in the karate network--nodes with higher degree have a higher tendency to be connected to nodes with lower degree. 
...
# compute the assortativity of this node attribute (random1)
...
Note that if your categorical labels are coded as integers (i.e., 1, 2, 3...) then you should not need the `factor()` function: `assortativity_nominal(karate_uu, types = V(karate_uu)$random2, directed = F)`.
...
Node pairs that do not have a path are simply ignored in the computation. This is the default behavior in `igraph`. Hence, this value reflects the mean of all "computable" short paths (i.e., when node pairs are found in the same component) and this is computed across all network components in the network.
...
Hence, it is good practice to first check if your network consists of a single component or of multiple components. We can use the following script to find out various bits of information: how many components, the size of each components (number of nodes), and obtain a list of nodes and their component membership. Basically, `components` is a special function that stores these pieces of information as its output, which you can then retrieve using the `$`: `csize` for size of component, `no` for number of components, `membership` for the component membership for each node.
