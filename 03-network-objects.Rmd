...
In practice, the networks that a social scientist is likely to be working with are going to be quite large, and the networks are derived from large datasets that are external to the R programming environment. In the next chapter we will explore how to get these datasets into RStudio and how to convert them into network. In this chapter, we focus on understanding the structure of the network object, a special datatype that `igraph` uses for network analysis. 

## Your first network 

The network object is a special variable that can only be analyzed with functions from the `igraph` library (that you previously installed). 
...
1. The IGRAPH label is a good sign, it means that this object is indeed a network object that `igraph` understands. This is followed by a UUID generated to uniquely identify the graph. 
2. The first letter in caps can either be **D** or **U**, which corresponds to *Directed* or *Undirected* edges, respectively. In this example we have a **U**, which means that the edges in this network are undirected. We will discuss what these are more specifically later (although you can probably guess what directedness is referring to!)--just keep in mind that these are two different kinds of edges.  
...
4. The third letter in caps: **W** or **-**, which corresponds to edges either having *Weights* or are unweighted, respectively. In this example we have a **-**, which means that the nodes are unweighted. Likewise, we will discuss weightedness as an edge attribute later--simply keep in mind that these are two different kinds of edges. 
...
It is a subtle change, but an additional **+** symbol is added to the dash. The function interprets **+** as the arrow head of the edge, and creates a *directed* edge pointing from 'cat' to 'bat'. *What do you think will happen if you typed in "+-" instead?*
...
As you can see, `V()` and `E()` are special functions in `igraph` which you can use to print a list of the nodes and edges associated with a particular network object. Take note that these are *capital* letters V and E, the code won't work if the letters are in lowercase. 
