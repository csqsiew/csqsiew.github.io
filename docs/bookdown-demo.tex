...
To download \texttt{igraph}, open RStudio and type the following into your Console (the panel with the ``\textgreater{}'' symbol - this is where you type in instructions for R to execute). By the way, if you hover your mouse over the code there will be a `copy' symbol for you to quickly copy and paste the code into RStudio.
...
\item
  Try installing the R package called \texttt{tidyverse}. \texttt{tidyverse} is a powerful set of packages that enable people to do cool and efficient data science things in R. It is highly recommended to learn it if you are interested in becoming a proficient user of R. See \url{https://www.tidyverse.org/} for more information.
\item
...
A good practice before starting any kind of programming foray is to create a dedicated folder on your computer for that project, and then create a new RStudio project that is associated with that folder. You can think of a ``project'' as a dedicated workspace for everything that you will do in that context, which makes it easy to save your progress and return to the same state on a different day.
...
\begin{quote}
\emph{Try this for yourself!} Set up a new RStudio project for a folder (e.g., make a sub-folder called `chapter01/' in the folder you have for this course) dedicated to this chapter and work through the contents, creating R scripts and data objects. Then, exit RStudio (remember to save the workspace image). Double-click on the .Rproj file to see if you are able to return to your previous workspace state.
\end{quote}
...
Notice the use of \texttt{\textless{}-} above: this is an \emph{assignment operator} to ``assign'' values to a name/label that you provide. (Note that you \emph{could} use \texttt{=} instead of \texttt{\textless{}-}, but the convention among R users is to use the arrow--I actually like that the arrow makes its more explicit that you are \emph{assigning} a value to a name/label.)
...
You should notice that while no output is printed to the console, there are new lines in the Environment tab of RStudio. Have a look and see if the items make sense to you. Basically, by running the above code, the value of \texttt{x} and \texttt{my\_height} (as well as \texttt{my\_name} and \texttt{SundayToday}) has been stored in the ``working memory'' of RStudio.
...
Notice that \texttt{x} is no longer found in the Environment tab.
...
Notice that you need to specify a \texttt{file} argument which is the name of the file that you want to save the information to, and that the file extension is \texttt{.RData}. This file is then saved to the same folder as your RStudio project.
...
Notice the use of a special function \texttt{c()} to combine single values into the vector. `c' for concatenate.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# editing / replacement}
...
All of your digital data in computers are stored in folders and sub-folders. When you want to open a file in a program like Microsoft Word, you need to navigate to the location where that specific file is stored.

Similarly, when programming in R, if you want to load datasets and/or previously saved workspaces into your current workspace you need to first ``point'' RStudio to the correct location where these files are stored.
...
First, the data should be saved as a \texttt{.csv} file. This is basically a ``text'' version of the Excel format (look at Save As\ldots{} in your Excel program). I recommend this because special packages and functions are needed to load data from proprietary file formats such as .xlsx. The csv format is much more flexible (it would still work with SPSS/JASP).

To see this in action, first create a .csv file called \texttt{numbers.csv} using your preferred spreadsheet program. Make two columns of random numbers and provide column headers. For column names, it is recommended to not leave any spaces and not to begin with a number. An example is shown below:
...
\subsection{Basic dataframe manipulation}\label{basic-dataframe-manipulation}

Now that you have a dataframe in your RStudio workspace, we can do things with it. Once you have \texttt{my\_data} in your workspace, try running the following code and see what you get.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# print the entire dataframe}
\NormalTok{my\_data }

\CommentTok{\# to view a specific column}
\NormalTok{my\_data}\SpecialCharTok{$}\NormalTok{ColumnA}

\CommentTok{\# to get a useful summary }
\FunctionTok{summary}\NormalTok{(my\_data)}

\CommentTok{\# to view the first and third rows }
\NormalTok{my\_data[}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{), ]}

\CommentTok{\# to view the first column}
\NormalTok{my\_data[ , }\DecValTok{1}\NormalTok{]}

\CommentTok{\# rename the first column name }
\FunctionTok{colnames}\NormalTok{(my\_data)[}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{\textquotesingle{}NetSciRocks\textquotesingle{}}

\CommentTok{\# get the mean of the second column}
\FunctionTok{mean}\NormalTok{(my\_data}\SpecialCharTok{$}\NormalTok{ColumnB)}
\end{Highlighting}
\end{Shaded}

Usually, you'll upload larger datasets and these manipulations may not be feasible (you won't want to print all 1,000 rows of your data!). But these give you a flavor of core programming concepts (e.g., \texttt{\$} for selection of some aspect of a more complex object, square brackets for subsetting, how basic functions like \texttt{summary()} work\ldots).

\section{Exercise}\label{exercise-1}

Find an old dataset of one of your introductory statistics courses. Convert it to a .csv file (if it is not in this format already), and try loading it into RStudio. A great way to learn is to take some of the code from this chapter and adapt it to explore your data frame.

\chapter{Chapter 3: Introduction to network objects}\label{ch3}

\section{What is a network?}\label{what-is-a-network}

A network consists of two sets of entities: A list of \emph{nodes} and a list of \emph{edges}. Nodes correspond to the individual elements whose pairwise relational structure is specified by edges or links. So, if node A and node B are related to each other based on some criteria set by the modeller (you!) then you would specify an edge between nodes A and B.

When networks are small (on the order of a handful of nodes), it would not be difficult to manually specify the list of nodes and their connections. However, this quickly gets onerous once the number of nodes increases because the number of possible edges increases exponentially. For instance, a network of 5 nodes could have a maximum of 10 edges, whereas a network of 50 nodes could have a maximum of 1,225 edges.

In practice, the networks that a social scientist is likely to be working with are going to be quite large, and the networks are derived from large datasets that are external to the R programming environment. In the next chapter we will explore how to get these datasets into RStudio and how to convert them into network. In this chapter, we focus on understanding the structure of the network object, a special datatype that \texttt{igraph} uses for network analysis.

\section{Your first network}\label{your-first-network}

The network object is a special variable that can only be analyzed with functions from the \texttt{igraph} library (that you previously installed).
...
\begin{verbatim}
## IGRAPH c54eb18 UN-- 2 1 -- 
## + attr: name (v/c)
## + edge from c54eb18 (vertex names):
## [1] cat--bat
\end{verbatim}
...
\tightlist
\item
  The IGRAPH label is a good sign, it means that this object is indeed a network object that \texttt{igraph} understands. This is followed by a UUID generated to uniquely identify the graph.
\item
  The first letter in caps can either be \textbf{D} or \textbf{U}, which corresponds to \emph{Directed} or \emph{Undirected} edges, respectively. In this example we have a \textbf{U}, which means that the edges in this network are undirected. We will discuss what these are more specifically later (although you can probably guess what directedness is referring to!)--just keep in mind that these are two different kinds of edges.\\
\item
...
\item
  The third letter in caps: \textbf{W} or \textbf{-}, which corresponds to edges either having \emph{Weights} or are unweighted, respectively. In this example we have a \textbf{-}, which means that the nodes are unweighted. Likewise, we will discuss weightedness as an edge attribute later--simply keep in mind that these are two different kinds of edges.
\item
...
\begin{verbatim}
## IGRAPH 92cb2fc DN-- 2 1 -- 
## + attr: name (v/c)
## + edge from 92cb2fc (vertex names):
## [1] cat->bat
\end{verbatim}

First, are you able to tell what was different from the code used to create the second network?

It is a subtle change, but an additional \textbf{+} symbol is added to the dash. The function interprets \textbf{+} as the arrow head of the edge, and creates a \emph{directed} edge pointing from `cat' to `bat'. \emph{What do you think will happen if you typed in ``+-'' instead?}
...
\begin{verbatim}
## + 3/3 vertices, named, from cc43dc5:
## [1] A B C
\end{verbatim}
...
\begin{verbatim}
## + 3/3 edges from cc43dc5 (vertex names):
## [1] A->B B->C C->A
\end{verbatim}

As you can see, \texttt{V()} and \texttt{E()} are special functions in \texttt{igraph} which you can use to print a list of the nodes and edges associated with a particular network object. Take note that these are \emph{capital} letters V and E, the code won't work if the letters are in lowercase.
...
All of the information associated with the connectivity structure of these 5 people can be represented in two ways: in an \emph{edge list} or an \emph{adjacency matrix}. The following sections will demonstrate what data in an edge list and data in an adjacency matrix look like, and how to import the data into RStudio so that it corresponds to the network representation that you want.

\section{Edge List}\label{edge-list}

An edge list is exactly what it says it is. A list of edges (connections) in the network. Below you can see that each edge is represented on a separate row and the name of each node in the pairwise relationship is listed in two separate columns.
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(igraph)}
\FunctionTok{library}\NormalTok{(igraphdata) }\CommentTok{\# karate is a network object available from this package}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data}\NormalTok{(}\StringTok{\textquotesingle{}karate\textquotesingle{}}\NormalTok{) }\CommentTok{\# notice that a karate object is in your environment }
...
In the \texttt{karate\_node\_added.csv} file, sample node attributes for the \texttt{karate} network have been added (it is as if I conducted a demographic survey with these individuals). The code below shows you how to read in this data and add this information as node attributes to the network using the \texttt{set\_vertex\_attribute} function.
...
\CommentTok{\# this code "arranges" the dataframe so that the information is ordered based on the node order in the network}
...
\begin{verbatim}
## IGRAPH 4b458a1 UNW- 34 78 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n)
\end{verbatim}
...
When you run \texttt{summary(karate)} you should see that there are additional labels (gender, belt, age) included in the attributes section.

\section{Adding edge attributes}\label{adding-edge-attributes}

Because there are usually more edges than nodes in the network, we usually do not manually insert edge attributes by hand (although it is possible!). In Chapter 4 recall that edge weights are usually included in the network at the point of network construction, as edge weights are simply in an additional column in the edge list that you are importing into RStudio.
...
\begin{verbatim}
##  [1] "same"      "same"      "same"      "same"      "different" "same"      "same"      "same"      "same"      "different" "different" "same"      "different"
## [14] "same"      "same"      "different" "same"      "same"      "same"      "same"      "different" "same"      "same"      "same"      "same"      "same"     
## [27] "same"      "same"      "same"      "different" "same"      "different" "same"      "different" "same"      "same"      "same"      "different" "different"
## [40] "same"      "different" "same"      "different" "same"      "same"      "same"      "same"      "different" "same"      "different" "different" "same"     
## [53] "same"      "different" "same"      "different" "same"      "same"      "different" "different" "different" "same"      "same"      "different" "different"
## [66] "different" "same"      "different" "different" "different" "same"      "same"      "different" "different" "same"      "same"      "different" "different"
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-46-2.pdf}}
...
\begin{verbatim}
## IGRAPH df81db7 UNW- 12 6 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}

How would you change the code to retain female individuals instead?
...
\begin{verbatim}
## IGRAPH 2236425 UNW- 10 21 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH d5b54db UNW- 26 59 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
Note that \texttt{make\_ego\_graph} returns a special ``list'' object. The \texttt{{[}{[}1{]}{]}} appended to it simply means that we want to extract the first element of the list, which is a network object. In the examples above, the list only contains a single network object. However, it is possible to extract the ego graphs of multiple individuals (e.g., \texttt{make\_ego\_graph(karate,\ order\ =\ 1,\ nodes\ =\ c(\textquotesingle{}Actor\ 3\textquotesingle{},\ \textquotesingle{}Actor\ 2\textquotesingle{}))}) which will give you a list of two network objects, the ego graph of Actor 3 and Actor 2. Then you can use {[}{[}1{]}{]} and {[}{[}2{]}{]} to extract the networks specifically.

\subsection{Subset by removing a group of nodes}\label{subset-by-removing-a-group-of-nodes}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{karate\_new }\OtherTok{\textless{}{-}} \FunctionTok{delete\_vertices}\NormalTok{(karate, }\AttributeTok{v =} \FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}John A\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Mr Hi\textquotesingle{}}\NormalTok{))}

\FunctionTok{plot}\NormalTok{(karate\_new, }\AttributeTok{vertex.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Notice how the network has fragmented into separate network components after the removal of these two nodes (and their edges).
...
\begin{verbatim}
## IGRAPH 9374212 DNW- 34 78 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH 580e095 DNW- 34 156 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH 2362bd6 DNW- 0 0 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH 24816e7 DNW- 34 156 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH cd3b0c5 DNW- 34 78 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
\begin{verbatim}
## IGRAPH e276238 DNW- 0 0 -- Zachary's karate club network
## + attr: name (g/c), Citation (g/c), Author (g/c), Faction (v/n), name (v/c), label (v/c), color (v/n), gender (v/c), belt (v/c), age (v/n), weight
## | (e/n), edge_type (e/c)
\end{verbatim}
...
Finally, below are some useful functions for network manipulations that you can learn more about by reading the documentation.
...
Let's continue working on Janelle's cat interaction network you've created in Chapter 4.

Janelle continued observing her cats and noticed that some pairs of cats interacted more frequently with other than other pairs of cats.
...
\def\labelenumi{\arabic{enumi}.}
\item
  How would you include this extra information about the frequency of interaction directly into the original raw data file? (Hint: The edge list approach is better for this, and you can put in some made up numbers from 1 to 10 to depict frequency of interaction).
\item
  Create a network object that includes this new information. (Hint: Is interaction frequency a node or edge attribute?)
\item
  Janelle has a .csv file containing the ages of the cats. How can you create a network object that includes this information? (Hint: Is cat age a node or edge attribute? Do you need to create a second file containing this information (feel free to make up some numeric data!)?)
\item
  Subset the network to only show the 4 oldest cats. How many edges does this network have?
\item
  Subset the network to only show cats that interacted frequently with each other. How many nodes and edges does this network have?
\item
  Create a network visualization of the entire cat network and make at least one modification to the default plot (see Chapter 14 for ideas). For instance, can you color the nodes based on the cat's ages?
\end{enumerate}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/shortest-path.jpeg}}
...
Because \texttt{karate\_uu} is undirected and unweighted, the result is unsurprisingly identical across all 4 versions (try this with a directed, weighted network!). When a graph contains edge weights, then they are automatically considered unless you indicate otherwise using \texttt{weights\ =\ NA}. Use the \texttt{directed} argument to specify if the direction of the edges should be considered in the computation of short paths. If this is selected, then only short paths whose directions are aligned are considered in the computation (A-\textgreater B-\textgreater C is a valid path connecting nodes A and C but not A-\textgreater B\textless-C).

An important point to note is that edge weights are interpreted as \emph{distances} rather than \emph{strength of connection}, as weighted short paths are computed by summing of the edge weights. Hence, a longer path is interpreted as being more distant, or further apart. See Chapter 7 for more information about this.
...
Another variant is to compute the \emph{average local clustering coefficient}, analogous to how mean degree is computed above. Below, we compute the local C (which is a measure of the level of interconnectedness among a target node's neighbors) for each node, then take the mean.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{transitivity}\NormalTok{(karate\_uu, }\AttributeTok{type =} \StringTok{\textquotesingle{}local\textquotesingle{}}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{mean}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
Humphries and Gurney (2008) considered a SWI larger than 1 as indicating the presence of a small world structure, since this implies that the target network has more local clustering than random expectation, while having relatively similar ASPL to the random graph. SWIs less than 1 or close to 1 indicates that there is little evidence to support the claim that the target network is a small world. Here, we have some evidence supporting the claim that our karate network has a small world structure. In practice, up to 1,000 random ER networks are generated to compute \(\overline L_{rand}\) and \(\overline C_{rand}\); although this can be a slow process depending on your network size. The number of random networks to generate can be adjusted in the for loop above.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/small-world.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/density.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/diameter.png}}
...
\textbf{Assortative mixing} is a measure of \emph{homophily} in the network, based on some internal/external property of the nodes. Do nodes that are connected to each other tend to have similar properties?

\subsection{Assortative Mixing by Degree (continuous attributes)}\label{assortative-mixing-by-degree-continuous-attributes}

A common extension of assortative mixing is \textbf{assortative mixing by degree}, which measures the correlation of degrees between neighboring nodes. Do nodes that are connected to each other tend to have similar degrees?

A positive value indicates that nodes that have many (few) neighbors tend to be connected to nodes that also have many (few) neighbors. A negative value indicates that nodes that are many (few) neighbors tend to be connected to nodes that have few (many) neighbors. Values very close to zero indicates no correlation.
...
There is \emph{dissortative mixing by degree} in the karate network--nodes with higher degree have a higher tendency to be connected to nodes with lower degree.
...
\CommentTok{\# compute the assortativity of this node attribute (random1)}
...
Note that if your categorical labels are coded as integers (i.e., 1, 2, 3\ldots) then you should not need the \texttt{factor()} function: \texttt{assortativity\_nominal(karate\_uu,\ types\ =\ V(karate\_uu)\$random2,\ directed\ =\ F)}.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-70-1.pdf}}
...
Node pairs that do not have a path are simply ignored in the computation. This is the default behavior in \texttt{igraph}. Hence, this value reflects the mean of all ``computable'' short paths (i.e., when node pairs are found in the same component) and this is computed across all network components in the network.
...
Hence, it is good practice to first check if your network consists of a single component or of multiple components. We can use the following script to find out various bits of information: how many components, the size of each components (number of nodes), and obtain a list of nodes and their component membership. Basically, \texttt{components} is a special function that stores these pieces of information as its output, which you can then retrieve using the \texttt{\$}: \texttt{csize} for size of component, \texttt{no} for number of components, \texttt{membership} for the component membership for each node.
...
The code below shows you how to extract the largest connected component of the sample network (basically a network subset based on component membership):
...
The network is the personal friendship network of a faculty of a UK university.
...
\begin{itemize}
\item
  How many nodes and edges does the undirected and unweighted version of this network have?
\item
...
##       16        9       10        6        3        4        4        5        2        5        2        2        2        3        2        2        5        3 
...
##        3        2        4        3        4        4        6       12        4        3        1        2        2        2        2       17
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/in_out_degree.jpg}}
...
\begin{verbatim}
##    Mr Hi  Actor 2  Actor 3  Actor 4  Actor 5  Actor 6  Actor 7  Actor 9 Actor 10 Actor 14 Actor 15 Actor 16 Actor 19 Actor 20 Actor 21 Actor 23 Actor 24 Actor 25 
##       45       23       33       16        4       12       11       16        5       17        6        7        5        9        2        9       22       11 
## Actor 26 Actor 27 Actor 28 Actor 29 Actor 30 Actor 31 Actor 32 Actor 33  Actor 8 Actor 11 Actor 12 Actor 13 Actor 18 Actor 22 Actor 17   John A 
##        7        4       13        9       12       11       17       31        8        5        1        7        7        3        5       53
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/images/hcc-lcc.jpg}}
...
## 0.1500000 0.3333333 0.2444444 0.6666667 0.6666667 0.5000000 0.5000000 0.5000000 0.0000000 0.6000000 1.0000000 1.0000000 1.0000000 0.3333333 1.0000000 1.0000000 
...
## 0.4000000 0.3333333 0.3333333 1.0000000 0.1666667 0.3333333 0.6666667 0.5000000 0.2000000 0.1969697 1.0000000 0.6666667 0.0000000 1.0000000 1.0000000 1.0000000 
##  Actor 17    John A 
## 1.0000000 0.1102941
\end{verbatim}
...
## 0.1500000 0.3333333 0.2444444 0.6666667 0.6666667 0.5000000 0.5000000 0.5000000 0.0000000 0.6000000 1.0000000 1.0000000 1.0000000 0.3333333 1.0000000 1.0000000 
##  Actor 24  Actor 25  Actor 26  Actor 27  Actor 28  Actor 29  Actor 30  Actor 31  Actor 32  Actor 33   Actor 8  Actor 11  Actor 12  Actor 13  Actor 18  Actor 22 
## 0.4000000 0.3333333 0.3333333 1.0000000 0.1666667 0.3333333 0.6666667 0.5000000 0.2000000 0.1969697 1.0000000 0.6666667       NaN 1.0000000 1.0000000 1.0000000 
##  Actor 17    John A 
## 1.0000000 0.1102941
\end{verbatim}
...
\begin{verbatim}
##      Mr Hi    Actor 2    Actor 3    Actor 4    Actor 5    Actor 6    Actor 7    Actor 9   Actor 10   Actor 14   Actor 15   Actor 16   Actor 19   Actor 20   Actor 21 
## 0.13629630 0.31521739 0.25925926 0.63750000 0.75000000 0.58333333 0.54545455 0.48437500 0.00000000 0.70588235 1.00000000 1.00000000 1.00000000 0.22222222 1.00000000 
##   Actor 23   Actor 24   Actor 25   Actor 26   Actor 27   Actor 28   Actor 29   Actor 30   Actor 31   Actor 32   Actor 33    Actor 8   Actor 11   Actor 12   Actor 13 
## 1.00000000 0.38636364 0.27272727 0.14285714 1.00000000 0.15384615 0.27777778 0.75000000 0.60606061 0.16470588 0.15542522 1.00000000 0.80000000        NaN 1.00000000 
...
\begin{verbatim}
##     Mr Hi   Actor 2   Actor 3   Actor 4   Actor 5   Actor 6   Actor 7   Actor 9  Actor 10  Actor 14  Actor 15  Actor 16  Actor 19  Actor 20  Actor 21  Actor 23 
## 0.2771084 0.2051282 0.2666667 0.2666667 0.5000000 0.4285714 0.3333333 0.4285714 0.5000000 1.0000000 0.3333333 0.2857143 0.6666667 0.2000000 1.0000000 0.2222222 
##  Actor 24  Actor 25  Actor 26  Actor 27  Actor 28  Actor 29  Actor 30  Actor 31  Actor 32  Actor 33   Actor 8  Actor 11  Actor 12  Actor 13  Actor 18  Actor 22 
## 0.2142857 0.3125000 0.4285714 0.4285714 0.5000000 0.3333333 0.4000000 0.2857143 0.5000000 1.0000000       NaN       NaN       NaN       NaN       NaN       NaN 
##  Actor 17    John A 
##       NaN       NaN
\end{verbatim}
...
\begin{verbatim}
##     Mr Hi   Actor 2   Actor 3   Actor 4   Actor 5   Actor 6   Actor 7   Actor 9  Actor 10  Actor 14  Actor 15  Actor 16  Actor 19  Actor 20  Actor 21  Actor 23 
## 0.7666667 0.6400000 0.7500000 0.8000000 0.7500000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 1.0000000 
##  Actor 24  Actor 25  Actor 26  Actor 27  Actor 28  Actor 29  Actor 30  Actor 31  Actor 32  Actor 33   Actor 8  Actor 11  Actor 12  Actor 13  Actor 18  Actor 22 
## 0.8571429 0.7142857 0.6000000 0.7500000 1.0000000 0.7500000 1.0000000 1.0000000 1.0000000 1.0000000       NaN       NaN       NaN       NaN       NaN       NaN 
##  Actor 17    John A 
##       NaN       NaN
\end{verbatim}
...
\begin{verbatim}
## [[1]]
## + 4/5 vertices, named, from d652b23:
## [1] A D E C
\end{verbatim}
...
\begin{verbatim}
## [[1]]
## + 3/5 vertices, named, from d652b23:
## [1] A B C
\end{verbatim}
...
\begin{verbatim}
##        Mr Hi      Actor 2      Actor 3      Actor 4      Actor 5      Actor 6      Actor 7      Actor 9     Actor 10     Actor 14     Actor 15     Actor 16     Actor 19 
## 0.0000000000 0.0021306818 0.0075757576 0.0040246212 0.0014204545 0.0000000000 0.0018939394 0.0016571970 0.0000000000 0.0023674242 0.0000000000 0.0000000000 0.0000000000 
##     Actor 20     Actor 21     Actor 23     Actor 24     Actor 25     Actor 26     Actor 27     Actor 28     Actor 29     Actor 30     Actor 31     Actor 32     Actor 33 
## 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0037878788 0.0000000000 0.0000000000 0.0018939394 0.0009469697 0.0026041667 0.0047348485 0.0056818182 
##      Actor 8     Actor 11     Actor 12     Actor 13     Actor 18     Actor 22     Actor 17       John A 
...
\begin{verbatim}
##        Mr Hi      Actor 2      Actor 3      Actor 4      Actor 5      Actor 6      Actor 7      Actor 9     Actor 10     Actor 14     Actor 15     Actor 16     Actor 19 
## 0.0000000000 0.0004734848 0.0083648990 0.0018939394 0.0000000000 0.0004734848 0.0014204545 0.0021306818 0.0001578283 0.0016571970 0.0000000000 0.0000000000 0.0000000000 
##     Actor 20     Actor 21     Actor 23     Actor 24     Actor 25     Actor 26     Actor 27     Actor 28     Actor 29     Actor 30     Actor 31     Actor 32     Actor 33 
## 0.0005523990 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0009469697 0.0000000000 0.0006313131 0.0020517677 0.0009469697 0.0007891414 0.0048137626 0.0001578283 
##      Actor 8     Actor 11     Actor 12     Actor 13     Actor 18     Actor 22     Actor 17       John A 
## 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0000000000 0.0000000000
\end{verbatim}
...
\begin{verbatim}
##       Mr Hi     Actor 2     Actor 3     Actor 4     Actor 5     Actor 6     Actor 7     Actor 9    Actor 10    Actor 14    Actor 15    Actor 16    Actor 19    Actor 20 
## 0.096997285 0.052876924 0.057078509 0.035859858 0.021977952 0.029111155 0.029111155 0.029766056 0.014309397 0.029536456 0.014535994 0.014535994 0.014535994 0.019604636 
##    Actor 21    Actor 23    Actor 24    Actor 25    Actor 26    Actor 27    Actor 28    Actor 29    Actor 30    Actor 31    Actor 32    Actor 33     Actor 8    Actor 11 
## 0.014535994 0.014535994 0.031522515 0.021076034 0.021006197 0.015044038 0.025639767 0.019573459 0.026288538 0.024590155 0.037158087 0.071693226 0.024490497 0.021977952 
##    Actor 12    Actor 13    Actor 18    Actor 22    Actor 17      John A 
## 0.009564745 0.014644892 0.014558677 0.014558677 0.016784005 0.100919182
\end{verbatim}
...
\begin{verbatim}
##      Mr Hi    Actor 2    Actor 3    Actor 4    Actor 5    Actor 6    Actor 7    Actor 9   Actor 10   Actor 14   Actor 15   Actor 16   Actor 19   Actor 20   Actor 21 
## 0.01523537 0.01552315 0.01938528 0.01886916 0.01581093 0.01667427 0.03091729 0.01884496 0.01729506 0.02746170 0.01523537 0.01523537 0.01523537 0.01701045 0.01523537 
##   Actor 23   Actor 24   Actor 25   Actor 26   Actor 27   Actor 28   Actor 29   Actor 30   Actor 31   Actor 32   Actor 33    Actor 8   Actor 11   Actor 12   Actor 13 
## 0.01523537 0.01523537 0.01523537 0.01935585 0.01523537 0.02484945 0.01798162 0.02141609 0.01939474 0.04512689 0.08402038 0.02351277 0.02484309 0.01552315 0.01988203 
##   Actor 18   Actor 22   Actor 17     John A 
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-108-1.pdf}}
...
\begin{verbatim}
## [1] 0.4197896 0.4197896 0.4155983 0.4155983 0.4155983
\end{verbatim}
...
\tightlist
\item
  Load the macaque brain network from the \texttt{igraphdata} library using the following code:
\end{enumerate}
...
\FunctionTok{data}\NormalTok{(}\StringTok{"macaque"}\NormalTok{) }\CommentTok{\# you should see a macaque object appear in your Environment}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Conduct a community detection analysis with the Infomap method and answer the following questions:
\end{enumerate}
...
\item
  What do these communities represent? (You will need to learn more about the network first! Try \texttt{?macaque})
\end{itemize}
...
\setcounter{enumi}{2}
\item
  Repeat the above with a different community detection method. How were the results similar or different?
\item
  Bonus question: What does the output of this code mean? \texttt{modularity(macaque,\ as.factor(V(macaque)\$shape))}
\end{enumerate}
...
\section{Erdos-Renyi Network Model}\label{erdos-renyi-network-model}
...
\textbf{Method 1}. Using the \texttt{sample\_gnm} function: where you need to specify the exact number of nodes (\texttt{n}) and the exact number of edges (\texttt{m}).

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\CommentTok{\# This creates a ER network with 10 nodes and 12 edges}
\NormalTok{ger1 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_gnm}\NormalTok{(}\AttributeTok{n =} \DecValTok{10}\NormalTok{, }\AttributeTok{m =} \DecValTok{12}\NormalTok{)}

\FunctionTok{plot}\NormalTok{(ger1)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-114-1.pdf}}

We'll want to create a set of ER networks, say, 100 of them, that have the same number of nodes and edges as our karate network. What the code does is to regenerate the ER network and save each instance in a list object called \texttt{ger100}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ger100 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{() }

\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{100}\NormalTok{) \{}
\NormalTok{  ger100[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_gnm}\NormalTok{(}\AttributeTok{n =} \FunctionTok{gorder}\NormalTok{(karate), }\AttributeTok{m =} \FunctionTok{gsize}\NormalTok{(karate)) }
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sapply}\NormalTok{(ger100, mean\_distance) }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{hist}\NormalTok{(}\AttributeTok{breaks =} \DecValTok{20}\NormalTok{, }\AttributeTok{xlab =} \StringTok{\textquotesingle{}Average Shortest Path Length\textquotesingle{}}\NormalTok{, }
       \AttributeTok{main =} \StringTok{\textquotesingle{}100 Erdos{-}Renyi Graphs\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-116-1.pdf}}

\textbf{Method 2}. Using the \texttt{sample\_gnp} function: where you need to specify the exact number of nodes (\texttt{n}) and the probability that an edge is created (\texttt{p}).

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\CommentTok{\# This creates a ER network with 10 nodes and 50\% chance that an edge is created }
\NormalTok{ger2 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_gnp}\NormalTok{(}\AttributeTok{n =} \DecValTok{10}\NormalTok{, }\AttributeTok{p =} \FloatTok{0.5}\NormalTok{)}

\FunctionTok{plot}\NormalTok{(ger2)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-117-1.pdf}}

To create the ER graphs for comparing against the karate network, we need to decide on a value of \texttt{p}. For the value of \texttt{p}, we'll use the network density of the karate network. Recall that network density is a measure of the number of actual edges to the number of possible edges--and so this can be used as a proxy for the probability that an edge is created among node pairs in the graph.

Notice that the ER graphs created using the \texttt{sample\_gnp} function leads to ER graphs having slightly different number of edges (and hence network density), whereas the edges of ER graphs created using the \texttt{sample\_gnm} function remains constant.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ger200 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{() }

\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{100}\NormalTok{) \{}
\NormalTok{  ger200[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_gnp}\NormalTok{(}\AttributeTok{n =} \FunctionTok{gorder}\NormalTok{(karate), }\AttributeTok{p =} \FunctionTok{edge\_density}\NormalTok{(karate)) }
\NormalTok{\}}

\FunctionTok{library}\NormalTok{(ggplot2)}
\CommentTok{\# we use ggplot2 to help make the visualizations }

\NormalTok{ger\_plot }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{edges =} \FunctionTok{c}\NormalTok{(}\FunctionTok{sapply}\NormalTok{(ger100, gsize), }\FunctionTok{sapply}\NormalTok{(ger200, gsize)),}
  \AttributeTok{density =} \FunctionTok{c}\NormalTok{(}\FunctionTok{sapply}\NormalTok{(ger100, edge\_density), }\FunctionTok{sapply}\NormalTok{(ger200, edge\_density)),}
  \AttributeTok{label =} \FunctionTok{c}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}gnm\textquotesingle{}}\NormalTok{, }\DecValTok{100}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}gnp\textquotesingle{}}\NormalTok{, }\DecValTok{100}\NormalTok{))}
\NormalTok{)}

\FunctionTok{ggplot}\NormalTok{(ger\_plot, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ label, }\AttributeTok{y =}\NormalTok{ edges)) }\SpecialCharTok{+} 
  \FunctionTok{stat\_boxplot}\NormalTok{(}\AttributeTok{geom =} \StringTok{"errorbar"}\NormalTok{, }\CommentTok{\# Error bars}
               \AttributeTok{width =} \FloatTok{0.25}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# Bars width}
  \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-118-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(ger\_plot, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ label, }\AttributeTok{y =}\NormalTok{ density)) }\SpecialCharTok{+} 
  \FunctionTok{stat\_boxplot}\NormalTok{(}\AttributeTok{geom =} \StringTok{"errorbar"}\NormalTok{, }\CommentTok{\# Error bars}
               \AttributeTok{width =} \FloatTok{0.25}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# Bars width}
  \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-118-2.pdf}}
...
\section{Watts-Strogatz Network Model}\label{watts-strogatz-network-model}
...
The model is initialized by a regular lattice network, where all nodes have the same degree to begin with. Below we have a network with 10 nodes (\texttt{size}) and each node is connected to 2 (\texttt{nei}) of its neighboring nodes. Because the rewiring probability is \texttt{p\ =\ 0}, we are not changing or rewiring the edges of this network yet. The lattice network is a simplified depiction of a network with high levels of local clustering.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{g }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\AttributeTok{dim =} \DecValTok{1}\NormalTok{, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{nei =} \DecValTok{2}\NormalTok{, }\AttributeTok{p =} \DecValTok{0}\NormalTok{)}
...
\begin{verbatim}
## IGRAPH 8dce5ce U--- 10 20 -- Watts-Strogatz random graph
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{transitivity}\NormalTok{(g, }\AttributeTok{type =} \StringTok{"average"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{degree}\NormalTok{(g) }\SpecialCharTok{|\textgreater{}} \FunctionTok{mean}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-119-1.pdf}}

The model states that as \texttt{p} increases, more of the edges in the lattice network are rewired in such a way that distant nodes becomes connected and it reduces the ASPL of the network.

Let's increase the rewiring probability and see how it changes the WS network:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{gr }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\AttributeTok{dim =} \DecValTok{1}\NormalTok{, }\AttributeTok{size =} \DecValTok{10}\NormalTok{, }\AttributeTok{nei =} \DecValTok{2}\NormalTok{, }\AttributeTok{p =} \FloatTok{0.3}\NormalTok{)}
...
\begin{verbatim}
## IGRAPH 2b7efa1 U--- 10 20 -- Watts-Strogatz random graph
## + attr: name (g/c), dim (g/n), size (g/n), nei (g/n), p (g/n), loops (g/l), multiple (g/l)
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean\_distance}\NormalTok{(gr)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{transitivity}\NormalTok{(gr, }\AttributeTok{type =} \StringTok{"average"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(gr, }\AttributeTok{layout =}\NormalTok{ layout\_in\_circle)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-120-1.pdf}}

We observe that both the ASPL and global clustering coefficient has decreased relative to the first network. When \texttt{p\ =\ 1} the network simply becomes a ER random network.

Below we will simulate the WS network model with the same number of nodes as the karate network, at varying rewiring probabilities \{0, 0.01, 0.05, 0.1, 1\}. We won't be able to fully match the WS network on the number of edges, but with \texttt{nei=2} the average degree of these networks will be 4 and relatively close to the average degree of the karate network which is 4.6.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{gws0 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\NormalTok{gws1 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\NormalTok{gws2 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\NormalTok{gws3 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\NormalTok{gws4 }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{) \{}
\NormalTok{  gws0[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(karate), }\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{  gws1[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(karate), }\DecValTok{2}\NormalTok{, }\FloatTok{0.01}\NormalTok{)}
\NormalTok{  gws2[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(karate), }\DecValTok{2}\NormalTok{, }\FloatTok{0.05}\NormalTok{)}
\NormalTok{  gws3[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(karate), }\DecValTok{2}\NormalTok{, }\FloatTok{0.1}\NormalTok{)}
\NormalTok{  gws4[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{sample\_smallworld}\NormalTok{(}\DecValTok{1}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(karate), }\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wsm }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{C =} \FunctionTok{c}\NormalTok{(}\FunctionTok{sapply}\NormalTok{(gws0, transitivity, }\AttributeTok{type =} \StringTok{\textquotesingle{}global\textquotesingle{}}\NormalTok{),}
        \FunctionTok{sapply}\NormalTok{(gws1, transitivity, }\AttributeTok{type =} \StringTok{\textquotesingle{}global\textquotesingle{}}\NormalTok{),}
        \FunctionTok{sapply}\NormalTok{(gws2, transitivity, }\AttributeTok{type =} \StringTok{\textquotesingle{}global\textquotesingle{}}\NormalTok{),}
        \FunctionTok{sapply}\NormalTok{(gws3, transitivity, }\AttributeTok{type =} \StringTok{\textquotesingle{}global\textquotesingle{}}\NormalTok{),}
        \FunctionTok{sapply}\NormalTok{(gws4, transitivity, }\AttributeTok{type =} \StringTok{\textquotesingle{}global\textquotesingle{}}\NormalTok{)}
\NormalTok{        ),}
  \AttributeTok{L =} \FunctionTok{c}\NormalTok{(}\FunctionTok{sapply}\NormalTok{(gws0, mean\_distance),}
        \FunctionTok{sapply}\NormalTok{(gws1, mean\_distance),}
        \FunctionTok{sapply}\NormalTok{(gws2, mean\_distance),}
        \FunctionTok{sapply}\NormalTok{(gws3, mean\_distance),}
        \FunctionTok{sapply}\NormalTok{(gws4, mean\_distance)),}
  \AttributeTok{p =} \FunctionTok{c}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}0\textquotesingle{}}\NormalTok{, }\DecValTok{20}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}0.01\textquotesingle{}}\NormalTok{, }\DecValTok{20}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}0.05\textquotesingle{}}\NormalTok{, }\DecValTok{20}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}0.1\textquotesingle{}}\NormalTok{, }\DecValTok{20}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{\textquotesingle{}1\textquotesingle{}}\NormalTok{, }\DecValTok{20}\NormalTok{))}
\NormalTok{)}

\FunctionTok{ggplot}\NormalTok{(wsm, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ p, }\AttributeTok{y =}\NormalTok{ C)) }\SpecialCharTok{+}
  \FunctionTok{stat\_boxplot}\NormalTok{(}\AttributeTok{geom =} \StringTok{"errorbar"}\NormalTok{, }\CommentTok{\# Error bars}
               \AttributeTok{width =} \FloatTok{0.25}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# Bars width}
  \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-122-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(wsm, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ p, }\AttributeTok{y =}\NormalTok{ L)) }\SpecialCharTok{+}
  \FunctionTok{stat\_boxplot}\NormalTok{(}\AttributeTok{geom =} \StringTok{"errorbar"}\NormalTok{, }\CommentTok{\# Error bars}
               \AttributeTok{width =} \FloatTok{0.25}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# Bars width}
  \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-122-2.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lc }\OtherTok{\textless{}{-}} \FunctionTok{layout\_in\_circle}\NormalTok{(gws0[[}\DecValTok{1}\NormalTok{]])}

\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{5}\NormalTok{), }\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)}\SpecialCharTok{+}\NormalTok{.}\DecValTok{1}\NormalTok{) }\CommentTok{\# space in top}

\FunctionTok{plot}\NormalTok{(gws0[[}\DecValTok{1}\NormalTok{]], }\AttributeTok{layout =}\NormalTok{ lc, }\AttributeTok{main=}\StringTok{\textquotesingle{}p=0\textquotesingle{}}\NormalTok{, }\AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(gws1[[}\DecValTok{1}\NormalTok{]], }\AttributeTok{layout =}\NormalTok{ lc, }\AttributeTok{main=}\StringTok{\textquotesingle{}p=0.01\textquotesingle{}}\NormalTok{, }\AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(gws2[[}\DecValTok{1}\NormalTok{]], }\AttributeTok{layout =}\NormalTok{ lc, }\AttributeTok{main=}\StringTok{\textquotesingle{}p=0.05\textquotesingle{}}\NormalTok{, }\AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(gws3[[}\DecValTok{1}\NormalTok{]], }\AttributeTok{layout =}\NormalTok{ lc, }\AttributeTok{main=}\StringTok{\textquotesingle{}p=0.1\textquotesingle{}}\NormalTok{, }\AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(gws4[[}\DecValTok{1}\NormalTok{]], }\AttributeTok{layout =}\NormalTok{ lc, }\AttributeTok{main=}\StringTok{\textquotesingle{}p=1\textquotesingle{}}\NormalTok{, }\AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-122-3.pdf}}

As shown in the visualization above, when \texttt{p\ =\ 0} the lattice network is regular, where each node has the same degree and is equidistant from all other nodes. When \texttt{p\ =\ 1} the network becomes fully disordered. In between, networks show the small world characteristic where they are more clustered than a random equivalent even though their path lengths are relatively short.

\section{Preferential Attachment Network Growth Model (Barabasi-Albert Network Model)}\label{preferential-attachment-network-growth-model-barabasi-albert-network-model}

The preferential attachment model was first proposed by Barabasi \& Albert to provide a mechanistic explanation of the power law degree distribution commonly observed in real world networks. In their model, the network begins with a single node and new nodes are added sequentially to the network. The new node \emph{preferentially} attaches to existing nodes in the network that have \emph{higher degree} (i.e., more edges). In this way, nodes that are already rich with edges, tend to acquire new edges from the incoming nodes. Over time, the final network has a degree distribution that follows a power law, where few nodes are hubs in the network, and many nodes have few connections.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plot\_nicely }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(network) \{ }
  \FunctionTok{set.seed}\NormalTok{(}\DecValTok{5}\NormalTok{) }\CommentTok{\# fix random seed for net viz algo }
  \FunctionTok{plot}\NormalTok{(network, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{, }\AttributeTok{edge.arrow.size =} \FloatTok{0.5}\NormalTok{, }\CommentTok{\# adjust arrow size, node frame color}
       \AttributeTok{layout =}\NormalTok{ layout\_with\_lgl, }\CommentTok{\# net viz layout algorithm  }
       \AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.size =} \FunctionTok{log}\NormalTok{(}\FunctionTok{degree}\NormalTok{(network))}\SpecialCharTok{*}\DecValTok{8}\NormalTok{, }\CommentTok{\# no node names, size is yoked to its degree}
       \AttributeTok{main =} \FunctionTok{paste0}\NormalTok{(}\StringTok{\textquotesingle{}\# nodes = \textquotesingle{}}\NormalTok{, }\FunctionTok{gorder}\NormalTok{(network))) }\CommentTok{\# label }
\NormalTok{\}}

\CommentTok{\# plot}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)}\SpecialCharTok{+}\NormalTok{.}\DecValTok{1}\NormalTok{, }\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{)) }
\CommentTok{\# reduce margins, ensure 2 rows and 2 columns }

\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}

\NormalTok{g1 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{10}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{)  }
\FunctionTok{plot\_nicely}\NormalTok{(g1)}

\NormalTok{g2 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{20}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{, }
                \AttributeTok{start.graph =}\NormalTok{ g1) }\CommentTok{\# use previous graph as the starter }
\FunctionTok{plot\_nicely}\NormalTok{(g2)}

\NormalTok{g3 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{30}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{, }\AttributeTok{start.graph =}\NormalTok{ g2)}
\FunctionTok{plot\_nicely}\NormalTok{(g3)}

\NormalTok{g4 }\OtherTok{\textless{}{-}} \FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{40}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{, }\AttributeTok{start.graph =}\NormalTok{ g3)}
\FunctionTok{plot\_nicely}\NormalTok{(g4)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-123-1.pdf}}
...
Below we explore the arguments in the \texttt{sample\_pa} function and show how they change the overall network structure. \texttt{n} refers to the number of nodes in the network. \texttt{m} refers to the number of new edges that an incoming node is given to attach to existing node(s). \texttt{power} is a parameter that weights the strength of the preferential attachment. If \texttt{power=1} then the probability of attaching to an existing node is a linear function of its degree. If \texttt{power} is larger than 1, existing nodes with higher degree are even more likely to attract new nodes. As seen in the visualization below, BA networks grown with a high power leads to a situation that only a small handful of nodes receive most of the new edges.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plot\_nicely2 }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(network, text) \{ }
  \FunctionTok{set.seed}\NormalTok{(}\DecValTok{5}\NormalTok{) }\CommentTok{\# fix random seed for net viz algo }
  \FunctionTok{plot}\NormalTok{(network, }\AttributeTok{vertex.frame.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{, }\AttributeTok{edge.arrow.size =} \FloatTok{0.5}\NormalTok{, }\CommentTok{\# adjust arrow size, node frame color}
       \AttributeTok{layout =}\NormalTok{ layout\_with\_lgl, }\CommentTok{\# net viz layout algorithm  }
       \AttributeTok{vertex.label =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{vertex.size =} \DecValTok{3}\SpecialCharTok{+}\FunctionTok{log}\NormalTok{(}\FunctionTok{degree}\NormalTok{(network))}\SpecialCharTok{*}\DecValTok{8}\NormalTok{, }\CommentTok{\# no node names, size is yoked to its degree}
       \AttributeTok{main =}\NormalTok{ text) }\CommentTok{\# label }
\NormalTok{\}}

\FunctionTok{par}\NormalTok{(}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)}\SpecialCharTok{+}\NormalTok{.}\DecValTok{1}\NormalTok{, }\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{)) }
\CommentTok{\# changing m, number of new edges }
\FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{20}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{plot\_nicely2}\NormalTok{(}\AttributeTok{text =} \StringTok{\textquotesingle{}m=1\textquotesingle{}}\NormalTok{)}
\FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{20}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{plot\_nicely2}\NormalTok{(}\AttributeTok{text =} \StringTok{\textquotesingle{}m=3\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-124-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{par}\NormalTok{(}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{)}\SpecialCharTok{+}\NormalTok{.}\DecValTok{1}\NormalTok{, }\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{)) }
\CommentTok{\# changing power, strength of the pref. attachment }
\FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{20}\NormalTok{, }\AttributeTok{power =} \DecValTok{1}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{plot\_nicely2}\NormalTok{(}\AttributeTok{text =} \StringTok{\textquotesingle{}power=1\textquotesingle{}}\NormalTok{)}
\FunctionTok{sample\_pa}\NormalTok{(}\AttributeTok{n=}\DecValTok{20}\NormalTok{, }\AttributeTok{power =} \DecValTok{3}\NormalTok{, }\AttributeTok{m =} \DecValTok{2}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{plot\_nicely2}\NormalTok{(}\AttributeTok{text =} \StringTok{\textquotesingle{}power=3\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-124-2.pdf}}

\section{Exercise}\label{exercise-9}

For Q1 and 2, we'll use the \texttt{UKfaculty} network from the \texttt{igraphdata} library.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(igraphdata)}
\FunctionTok{data}\NormalTok{(}\StringTok{"UKfaculty"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Compare the ASPL and global clustering coefficient of the \texttt{UKfaculty} network against the \emph{Erdos-Renyi} random network model. What do your results imply?
\item
  Compare the community structure of the \texttt{UKfaculty} network against the \emph{Erdos-Renyi} random network model. What do your results imply?
\item
  Let's assume that Mark conducted an analysis of the word association network and found that (i) the degree distribution followed a power law with an exponent of \textasciitilde3 and (ii) it had a modularity, Q, of 0.4. How would Mark use the preferential attachment model to create a generative network with a similar degree distribution and compare its modularity to the word association network?
\item
  What happens when \texttt{power} in the \texttt{sample\_pa} function is set to less than 1 (but larger than 0)? How would you test the behavior of this parameter?
\end{enumerate}
...
##  [1] spike --speak  spike --spoke  spike --speck  spike --spook  spoke --spook  spoke --speck  speak --spoke  speck --spook  speak --spook  speak --speck  speck --sped  
## [12] sped  --speed  sped  --spud   speed --spud   speed --stead  speed --seed   speed --speech speak --speed  stead --seed   seed  --seek   seek  --sneak  seek  --sleek 
## [23] speak --seek   seek  --peek   sneak --sleek  speak --sneak  speak --sleek  speak --peek   speak --speech speech--peach  peach --peace  peek  --peach  peach --peep  
## [34] peach --pea    peach --peat   peach --peel   peach --peas   peach --teach  peach --leach  peach --beach  peach --each   peach --pouch  peach --pitch  peach --patch 
## [45] peach --pooch  peach --poach  peach --preach peach --reach  pea   --peas   pea   --peel   peep  --pea    peek  --pea    peace --pea    pea   --peat   peep  --peel  
## [56] peep  --peas   peep  --peat   peace --peep   peek  --peep   peel  --peas   peat  --peel   peace --peel   peek  --peel   peat  --peas   peace --peas   peek  --peas  
## [67] peek  --peat   peek  --peace  peace --peat   teach --leach  leach --reach  leach --each   leach --beach  teach --beach  teach --each   teach --reach  beach --each  
## [78] beach --reach  each  --reach  preach--reach  peach --perch  pouch --perch  pitch --perch  pooch --perch  poach --perch  patch --perch  pouch --patch  pitch --patch 
...
\begin{verbatim}
##           V1           V2           V3          V3A           V4          V4t          VOT           VP           MT       MSTd/p         MSTl           PO          LIP 
## 0.0003156566 0.0003310887 0.0002976190 0.0003108003 0.0005071549 0.0003282828 0.0002693603 0.0002976190 0.0002946128 0.0002921371 0.0003328742 0.0002861953 0.0003135522 
##          PIP          VIP           DP           7a          FST         PITd         PITv         CITd         CITv         AITd         AITv         STPp         STPa 
## 0.0003507295 0.0005250922 0.0003086420 0.0003900112 0.0002702463 0.0003367003 0.0002840909 0.0004349046 0.0003682660 0.0003198653 0.0003848004 0.0003661616 0.0004377104 
##           TF           TH          FEF           46           3a           3b            1            2            5           Ri          SII           7b            4 
## 0.0004307784 0.0003437149 0.0003979186 0.0004861111 0.0004349046 0.0004629630 0.0003787879 0.0003749617 0.0003979186 0.0004103535 0.0005536131 0.0003561254 0.0003114478 
##            6          SMA           Ig           Id           35           36 
## 0.0003137435 0.0003156566 0.0004278900 0.0004945286 0.0005471380 0.0005611672
\end{verbatim}
...
##  2.000000  6.200000  5.428571  4.076923 12.900000  3.200000  3.000000  6.000000  6.125000  7.941176  4.272727  5.933333 10.700000  3.888889 12.333333  4.833333 
##        7a       FST      PITd      PITv      CITd      CITv      AITd      AITv      STPp      STPa        TF        TH       FEF        46        3a        3b 
##  8.866667 10.157895  5.250000  6.666667  3.000000  3.750000  5.600000  3.000000  6.400000  1.800000 11.941176  7.333333 13.454545 13.900000  2.000000  1.000000 
##         1         2         5        Ri       SII        7b         4         6       SMA        Ig        Id        35        36 
##  2.750000  4.636364  5.181818  1.500000  7.615385  6.538462  4.200000  4.272727  2.750000  4.000000  2.500000  3.000000  4.666667
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-139-1.pdf}} \pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-139-2.pdf}}
...
\begin{verbatim}
##  [1] "AVOID REMINDERS OF THE TRAUMA"  "BAD DREAMS ABOUT THE TRAUMA"    "BEING JUMPY OR EASILY STARTLED" "BEING OVER ALERT"               "DISTANT OR CUT OFF FROM PEOPLE"
##  [6] "FEELING EMOTIONALLY NUMB"       "FEELING IRRITABLE"              "FEELING PLANS WONT COME TRUE"   "HAVING TROUBLE CONCENTRATING"   "HAVING TROUBLE SLEEPING"       
## [11] "LESS INTEREST IN ACTIVITIES"    "NOT ABLE TO REMEMBER"           "NOT THINKING ABOUT TRAUMA"      "PHYSICAL REACTIONS"             "RELIVING THE TRAUMA"           
...
\begin{verbatim}
##            AROTT      BDATT      BJOES       BOA    DOCOFP       FEN       FEI     FPWCT        HTC       HTS      LIIA      NATR      NTAT       PHR       RTT
## AROTT  0.0000000  0.0000000  0.0000000 0.1555554 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.1515577 0.1746399 0.0000000
## BDATT  0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 -0.1221041 0.0000000 0.0000000 0.0000000 0.0000000 0.1089284 0.0000000
## BJOES  0.0000000  0.0000000  0.0000000 0.4075190 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.1129132 0.0000000 0.1204094 0.1692879 0.0000000
## BOA    0.1555554  0.0000000  0.4075190 0.0000000 0.0000000 0.1423142 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000
## DOCOFP 0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.2357181 0.1334469 0.0000000  0.0000000 0.0000000 0.3509726 0.0000000 0.0000000 0.0000000 0.0000000
## FEN    0.0000000  0.0000000  0.0000000 0.1423142 0.2357181 0.0000000 0.1410481 0.1474119  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000
## FEI    0.0000000  0.0000000  0.0000000 0.0000000 0.1334469 0.1410481 0.0000000 0.1214582  0.1793871 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000
## FPWCT  0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.1474119 0.1214582 0.0000000  0.0000000 0.0000000 0.2044012 0.0000000 0.0000000 0.0000000 0.0000000
## HTC    0.0000000 -0.1221041  0.0000000 0.0000000 0.0000000 0.0000000 0.1793871 0.0000000  0.0000000 0.1332300 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000
## HTS    0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.1332300 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000
## LIIA   0.0000000  0.0000000  0.1129132 0.0000000 0.3509726 0.0000000 0.0000000 0.2044012  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.1093724
## NATR   0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.1711055 0.0000000 0.1487162
## NTAT   0.1515577  0.0000000  0.1204094 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.1711055 0.0000000 0.0000000 0.0000000
## PHR    0.1746399  0.1089284  0.1692879 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.1811024
## RTT    0.0000000  0.0000000  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.1093724 0.1487162 0.0000000 0.1811024 0.0000000
## UWROT  0.0000000  0.1461690 -0.1109757 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.0000000 0.0000000 0.1546641 0.1169038 0.0000000 0.0000000
## UTOI   0.0000000  0.1710409  0.0000000 0.0000000 0.0000000 0.0000000 0.0000000 0.0000000  0.0000000 0.1351728 0.0000000 0.0000000 0.0000000 0.0000000 0.1970405
##             UWROT      UTOI
## AROTT   0.0000000 0.0000000
## BDATT   0.1461690 0.1710409
## BJOES  -0.1109757 0.0000000
## BOA     0.0000000 0.0000000
## DOCOFP  0.0000000 0.0000000
## FEN     0.0000000 0.0000000
## FEI     0.0000000 0.0000000
## FPWCT   0.0000000 0.0000000
## HTC     0.0000000 0.0000000
## HTS     0.0000000 0.1351728
## LIIA    0.0000000 0.0000000
## NATR    0.1546641 0.0000000
## NTAT    0.1169038 0.0000000
## PHR     0.0000000 0.0000000
## RTT     0.0000000 0.1970405
## UWROT   0.0000000 0.4497070
## UTOI    0.4497070 0.0000000
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/test-1.pdf}}
...
This chapter aims to introduce you the basics of network visualization. Note that for a very large network (\textgreater{} 100 nodes) it becomes very difficult to create good visualizations using basic \texttt{igraph} plotting functions. You may need to venture to other R packages (see Chapter 15 - under development). A nice open-source and user-friendly (no coding needed!) software for creating great network visualizations is \href{https://gephi.org/}{Gephi}.
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-151-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-152-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-153-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-153-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-154-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-155-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(karate, }\AttributeTok{vertex.size =} \FloatTok{0.5}\SpecialCharTok{*}\FunctionTok{V}\NormalTok{(karate)}\SpecialCharTok{$}\NormalTok{age) }\CommentTok{\# you will have to play around to find a suitable scale, e.g. normalize the continuous variable }
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-155-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-156-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-157-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-158-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-158-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-159-1.pdf}}

\subsection{Arrows for directed edges}\label{arrows-for-directed-edges}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(}\FunctionTok{as.directed}\NormalTok{(karate, }\AttributeTok{mode =} \StringTok{\textquotesingle{}random\textquotesingle{}}\NormalTok{), }\AttributeTok{vertex.color =} \StringTok{\textquotesingle{}white\textquotesingle{}}\NormalTok{, }\AttributeTok{edge.arrow.width =} \FloatTok{0.8}\NormalTok{, }\AttributeTok{edge.arrow.size =} \FloatTok{0.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-160-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-161-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-161-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-162-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-163-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-164-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-168-1.pdf}}
...
\begin{verbatim}
## Expected total run time for 1000 sims, using 4 threads is 402 seconds.
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-169-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{bookdown-demo_files/bookdown-demo_files/figure-latex/unnamed-chunk-169-2.pdf}}
