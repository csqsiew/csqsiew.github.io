...
Notice the use of `<-` above: this is an *assignment operator* to "assign" values to a name/label that you provide. (Note that you *could* use `=` instead of `<-`, but the convention among R users is to use the arrow--I actually like that the arrow makes its more explicit that you are *assigning* a value to a name/label.)
...
Notice that `x` is no longer found in the Environment tab. 
...
Notice that you need to specify a `file` argument which is the name of the file that you want to save the information to, and that the file extension is `.RData`. This file is then saved to the same folder as your RStudio project. 
...
Notice the use of a special function `c()` to combine single values into the vector. 'c' for concatenate. 
...
To see this in action, first create a .csv file called `numbers.csv` using your preferred spreadsheet program. Make two columns of random numbers and provide column headers. For column names, it is recommended to not leave any spaces and not to begin with a number. An example is shown below: 
...
Now that you have a dataframe in your RStudio workspace, we can do things with it. Once you have `my_data` in your workspace, try running the following code and see what you get. 
...
# to view the first and third rows 
my_data[c(1,3), ]
...
colnames(my_data)[1] <- 'NetSciRocks'

# get the mean of the second column
mean(my_data$ColumnB)
```

Usually, you'll upload larger datasets and these manipulations may not be feasible (you won't want to print all 1,000 rows of your data!). But these give you a flavor of core programming concepts (e.g., `$` for selection of some aspect of a more complex object, square brackets for subsetting, how basic functions like `summary()` work...).  
