...
1. Load the macaque brain network from the `igraphdata` library using the following code: 
...
data("macaque") # you should see a macaque object appear in your Environment
```

2. Conduct a community detection analysis with the Infomap method and answer the following questions:
...
- What do these communities represent? (You will need to learn more about the network first! Try `?macaque`)

3. Repeat the above with a different community detection method. How were the results similar or different? 

4. Bonus question: What does the output of this code mean? `modularity(macaque, as.factor(V(macaque)$shape))`
